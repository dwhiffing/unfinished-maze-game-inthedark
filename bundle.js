...
(function () {
  var game = new Phaser.Game(800, // width
  450, // height
  ...
  _createClass(InputManager, [{
    ...
    value: function update() {
      ...
      Object.keys(this.bindings).forEach(function (key) {
        if (_this.keys[key] && _this.keys[key].justDown) {
          _this.bindings[key](_this.game);
        }
      });
    }
  }]);
...
var MapGenerator = function () {
  function MapGenerator(game, size, stepByStep) {
    ...
    for (var i = 0; i < this.size * this.size; i++) {
      ...
      this.data[i] = new _tile2.default(x, y);
    }
    this.generate();
  }
  // generate a new map
  ...
  _createClass(MapGenerator, [{
    ...
    key: 'rejigger',
    value: function rejigger() {
      var _this = this;

      var blankTilesWithNonBlankNeighbours = this.getBlankTiles().filter(function (blank) {
        var nonBlankNeighbours = _this.getTileNeighbours(blank).filter(function (tile) {
          return tile && tile.type !== -1;
        });
        return nonBlankNeighbours.length > 0;
      });
      var tileToChange = _lodash2.default.sample(blankTilesWithNonBlankNeighbours);
    ...
    value: function placeNextTile() {
      ...
      do {
        var openTiles = this.getOpenTiles();
        openTile = _lodash2.default.sample(openTiles);
        if (!openTile) {
          return;
        }
        var unconnectedPath = this.getUnconnectedPaths(openTile)[0];
        var neighbours = this.getTileNeighbours(openTile);
        nextTile = neighbours[_utils2.default.invert(unconnectedPath)];
        count2++;
        if (count2 > 10) return;
      } while (!nextTile);
      ...
      do {
        nextTile.type = -1;
        var connectingPaths = this.getPathsConnectingToTile(nextTile);
    ...
    key: 'getPathsConnectingToTile',
    value: function getPathsConnectingToTile(tile) {
      var _this2 = this;

      var neighbours = this.getTileNeighbours(tile);
      return neighbours.filter(function (neighbour, direction) {
        if (!neighbour) return false;
        var neighbourUnconnectedPaths = _this2.getUnconnectedPaths(neighbour);
        return neighbourUnconnectedPaths.some(function (path) {
    ...
    key: 'tileHasBlockedPath',
    value: function tileHasBlockedPath(tile) {
      var neighbours = this.getTileNeighbours(tile);
      var paths = this.getUnconnectedPaths(tile);
      var blockedPaths = paths.some(function (path) {
        var tilePathLeadsTo = neighbours[_utils2.default.invert(path)];
    ...
    value: function getTileAt(x, y) {
      ...
      var tile = this.data[x + y * this.size];
      return tile;
    }
    ...
    key: 'getOpenTiles',
    value: function getOpenTiles() {
      var _this3 = this;

      return this.data.filter(function (tile) {
        if (tile.type === -1) return false;
        return _this3.hasUnconnectedPaths(tile);
      });
    }
    // does a tile have any paths that are not connected

  }, {
    key: 'hasUnconnectedPaths',
    value: function hasUnconnectedPaths(tile) {
      return this.getUnconnectedPaths(tile).length > 0;
    }
    // get all the unconnected paths of a tile

  }, {
    key: 'getUnconnectedPaths',
    value: function getUnconnectedPaths(tile) {
      var _this4 = this;

      var openPaths = _utils2.default.invert(tile.openPaths());
      var neighbours = this.getTileNeighbours(tile);
      var unconnectedPaths = openPaths.filter(function (path) {
        var neighbour = neighbours[path];
        if (!neighbour) return true;
        return !_this4.tilesConnect(tile, neighbour);
      });
      return _utils2.default.invert(unconnectedPaths);
    }
    // are the two given tiles neighbouring?

  }, {
    key: 'tilesAreNeighbours',
    value: function tilesAreNeighbours(tileA, tileB) {
      var aNeighbours = this.getTileNeighbours(tileA);
      if (!aNeighbours.some(function (tile) {
        return tile === tileB;
      })) {
        return false;
      }
    ...
    key: 'tilesConnect',
    value: function tilesConnect(tileA, tileB) {
      if (!this.tilesAreNeighbours(tileA, tileB)) {
        return false;
      }
      var aNeighbours = this.getTileNeighbours(tileA);
      var directionFromAtoB = aNeighbours.indexOf(tileB);
      var aPaths = tileA.openPaths();
      var bPaths = tileB.openPaths();
      switch (directionFromAtoB) {
        case 0:
          // tile B is north of A
          return aPaths.some(function (p) {
            return p === 2;
          }) && bPaths.some(function (p) {
            return p === 0;
          });
        case 1:
          // tile B is east of A
    ...
    value: function getTileNeighbours(tile) {
      var clean = arguments.length <= 1 || arguments[1] === undefined ? false : arguments[1];

      if (!tile) {
        return null;
      }
      var neighbours = [this.getTileAt(tile.x, tile.y - 1), this.getTileAt(tile.x + 1, tile.y), this.getTileAt(tile.x, tile.y + 1), this.getTileAt(tile.x - 1, tile.y)];
      if (clean) {
        neighbours = neighbours.filter(function (tile) {
          return !!tile;
        });
      }
      return neighbours;
    ...
    value: function getCenterTile() {
      var center = Math.floor(this.size * this.size / 2);
...
var _map = require('../entities/map');

var _map2 = _interopRequireDefault(_map);

var _miniMap = require('../entities/miniMap');

var _miniMap2 = _interopRequireDefault(_miniMap);

var _lightManager = require('../entities/lightManager');
...
var _InputManager = require('../entities/InputManager');
...
exports.default = {
  create: function create(game) {
    ...
    game.map = new _map2.default(game, 5);
    game.miniMap = new _miniMap2.default(game, game.map);
...
exports.default = {
  ...
  toWords: function toWords(path) {
    var _this = this;

    if (Array.isArray(path)) {
      return path.map(function (p) {
        return _this._directionNumToWord(p);
      });
    }
